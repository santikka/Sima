...
stroke_mortality <- ManipulationEvent$new(
    ...
    mechanism = expression({
        status[living & stroke == 1L, alive := rbern(ifelse(age - age_at_stroke < 28.0 / 365.0, 0.72 ^ (1 / 28), expit(par[1] + par[2] * exp(((age - age_at_stroke) * 365 - 27) * par[3]))))]
    })
)
...
initializer <- function(n) {
    ...
    smoking <- sex_age_init(n, sex_init, age_init, c(0.319, 0.298, 0.278, 0.219, 0.109, 0.016), c(0.229, 0.193, 0.184, 0.147, 0.054, 0.016), 30, 80, 10)
    diabetes <- sex_age_init(n, sex_init, age_init, c(0.012, 0.045, 0.110, 0.209, 0.244, 0.220), c(0.022, 0.032, 0.056, 0.126, 0.172, 0.204), 30, 80, 10)
    parent_stroke <- rbin(n, prob = 0.131)
    high_glucose <- sex_age_init(n, sex_init, age_init, c(0.013, 0.043, 0.080, 0.114, 0.128, 0.110), c(0.019, 0.012, 0.030, 0.055, 0.121, 0.093), 30, 80, 10)
    stroke <- rep(0L, n)
    ...
    gen_grp$waist_circumference <- c(93.7, 97.4, 102.0, 103.1, 101.3, 100.3, 83.4, 87.8, 91.7, 92.9, 93.9, 93.8)
